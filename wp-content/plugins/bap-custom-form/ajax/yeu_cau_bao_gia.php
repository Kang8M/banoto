<?php
add_action('wp_ajax_yeu_cau_bao_gia', 'yeu_cau_bao_gia_init');
add_action('wp_ajax_nopriv_yeu_cau_bao_gia', 'yeu_cau_bao_gia_init');
function yeu_cau_bao_gia_init()
{
    $data = "NOT_OK";
    global $wpdb;
    if (isset($_POST['ho_ten'])) {
        ...
        if (!empty($hoten) && !empty($sanpham) && !empty($link) && !empty($khuvuc) && !empty($hinhthuc) && !empty($dienthoai) && !empty($email)) {
            ...
            if ($wpdb->query($sql)) {
                ...
                sendEmail('Dang Ky: Yeu Cau Bao Gia', $html);
            }
        }
    }
