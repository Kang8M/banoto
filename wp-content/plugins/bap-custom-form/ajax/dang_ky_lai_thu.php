<?php
add_action('wp_ajax_dang_ky_lai_thu', 'dang_ky_lai_thu_init');
add_action('wp_ajax_nopriv_dang_ky_lai_thu', 'dang_ky_lai_thu_init');
function dang_ky_lai_thu_init()
{
    $data = "NOT_OK";
    global $wpdb;
    if (isset($_POST['ho_ten'])) {
        ...
        if (!empty($hoten) && !empty($sanpham) && !empty($link) && !empty($khuvuc) && !empty($hinhthuc) && !empty($dienthoai) && !empty($email)) {
            ...
            if ($wpdb->query($sql)) {
                ...
                sendEmail('Dang Ky: Dang Ky Lai Thu', $html);
            }
        }
    }
